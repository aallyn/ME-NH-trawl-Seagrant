...
# Path to "RES_Data"
shapefile_root <- "/Users/aallyn/Library/CloudStorage/Box-Box/RES_Data/"
res_root <- "/Users/aallyn/Library/CloudStorage/Box-Box/Mills Lab/Projects/"
...
# Creating absences
species_keep <- c("lobster american", "cod atlantic", "flounder winter", "haddock", "hake atlantic red", "hake silver (whiting)", "hake white", "monkfish", "pollock", "cunner", "redfish acadian ocean perch", "wolffish atlantic", "cusk", "dogfish spiny", "skate winter", "skate little", "sculpin longhorn", "sea bass black", "alewife", "herring atlantic", "sand lance american", "capelin", "squid long finned", "butterfish", "mackerel atlantic")
...
var_checks <- tows_out %>%
  ...
  mutate(.,
    "SeasonYearFactor" = factor(Survey, levels = c("FL00", "SP01", "FL01", "SP02", "FL02", "SP03", "FL03", "SP04", "FL04", "SP05", "FL05", "SP06", "FL06", "SP07", "FL07", "SP08", "FL08", "SP09", "FL09", "SP10", "FL10", "SP11", "FL11", "SP12", "FL12", "SP13", "FL13", "SP14", "FL14", "SP15", "FL15", "SP16", "FL16", "SP17", "FL17", "SP18", "FL18", "SP19", "FL19")),
...
# Save it
saveRDS(tidy_mod_data, here("Data/tidy_mod_data.rds"))
```
...
```{r, eval = eval_use}
# Read in the data
tidy_mod_data <- readRDS(here("Data/tidy_mod_data.rds"))
...
# Making the extrapolation grid and visualizing the sub regions -- USE NEWEST FUNCTION!!
vast_extrap_grid <- vast_make_extrap_grid(region_shapefile = menh_chull, index_shapes = menh_regions_out, cell_size = 4000) %>%
  drop_na(., STRATA)
...
out_dir_use <- paste0(res_root, "ME-NH-trawl-Seagrant/Objective 3/Results/") # Box path update!!!!!
...
for (i in seq_along(species_keep)) {
  # for (i in 23:length(species_keep)) {
  ...
  vast_sample_data <- data.frame("Year" = tidy_mod_data_run$Year, "Lat" = tidy_mod_data_run$DECDEG_BEGLAT, "Lon" = tidy_mod_data_run$DECDEG_BEGLON, "Biomass" = tidy_mod_data_run$BIOMASS, "Swept" = tidy_mod_data_run$Tow_LengthNM, "Pred_TF" = ifelse(tidy_mod_data_run$Year < pred_year_start, 0, 1))
  ...
  # Model formula
  gam_degree <- 2
  hab_formula <- ~ bs(Depth, degree = 2, intercept = FALSE) + bs(SST_seasonal, degree = 2, intercept = FALSE) + bs(BT_seasonal, degree = 2, intercept = FALSE)
  ...
  if (class(vast_fitted_hab_covs) == "fit_model" && (max(abs(vast_fitted_hab_covs$parameter_estimates$diagnostics$final_gradient)) <= 0.01)) {
    ...
    vast_density_plot <- vast_fit_plot_density(vast_fit = vast_fitted_hab_covs, nice_category_names = nice_category_names, mask = menh_chull, all_times = as.character(unique(vast_sample_data$Year)), plot_times = NULL, land_sf = land, xlim = xlim_use, ylim = ylim_use, x_dim_length = 715, y_dim_length = 733, lab_lon = -67.5, lab_lat = 43, panel_or_gif = "panel", out_dir = out_dir_use, land_color = "#d9d9d9", panel_cols = 4, panel_rows = 5)

    # Extracting stratified biomass indices and plotting as a time series
    biomass_indices <- get_vast_index_timeseries(vast_fit = vast_fitted_hab_covs, all_times = unique(vast_sample_data$Year), nice_category_names = nice_category_names, index_scale = c("log"), out_dir = out_dir_use)

    plot_vast_index_timeseries(index_res_df = biomass_indices, index_scale = "log", nice_category_names = nice_category_names, nice_xlab = "Year", nice_ylab = "Log biomass index (metric tons)", paneling = "none", color_pal = NULL, out_dir = out_dir_use)

    # Extracting center of gravity
    cog_df <- vast_get_cog(vast_fit = vast_fitted_hab_covs, all_times = unique(vast_sample_data$Year), nice_category_names = nice_category_names, out_dir = out_dir_use)
    cog_plot <- plot_cog(cog_df = cog_df, df_crs = "+proj=utm +zone=19 +datum=WGS84 +units=km +no_defs", plot_crs = 4326, summarize = TRUE, land_sf = land, xlim = xlim_use, ylim = ylim_use, nice_category_names = nice_category_names, land_color = "#d9d9d9", color_pal = NULL, out_dir = out_dir_use)
  } else {
    progress_new <- "Model failed"
    write(progress_new, file = paste(out_dir_use, species_keep[i], "_progress.txt", sep = ""), append = TRUE)

    # # Edits....
    # Find which parameters have problematic gradients and make adjustments?
...
mod_fits <- list.files(path = out_dir_use, pattern = "fitted_vast.rds", full.names = TRUE)
...
for (i in seq_along(mod_fits)) {
  ...
  if(mod_temp$Report == "Model is not converged"){
    next()
  }
  ...
  names_new <- c(
    paste(sub("_fitted_vast.rds", "", sub(".*/", "", mod_fits[[i]])), "R1_gct", sep = "_"),
  ...
  names_new <- c(
    paste(sub("_fitted_vast.rds", "", sub(".*/", "", mod_fits[[i]])), "D_gct", sep = "_"),
...
pred_prey_bio_out <- pred_prey_bio_df %>%
  group_by(year) %>%
  mutate(.,
    # Lob_Cod_LocalColloc = loc_collocfn(prey = `lobster american_Density`, pred = `cod atlantic_Density`),
  ...
  group_by(., year) %>%
  summarize_at(., vars(Lob_Cod_SchoBio:Lob_WinterSkate_AB), mean, na.rm = TRUE)
...
bio_ovr_plots <- ggplot() +
  ...
  theme_bw()
ggsave(bio_ovr_plots, file = paste(out_dir_use, "BiomassOverlapSummaries", ".jpg", sep = ""))
...
pred_prey_pres_out <- pred_prey_pres_df %>%
  group_by(year) %>%
  mutate(.,
    # Lob_Cod_AreaOvr = area_overlapfn(prey = `lobster american_Presence_Absence`, pred = `cod atlantic_Presence_Absence`, area = area),
  ...
  group_by(., year) %>%
  summarize_at(., vars(Lob_Cod_SchoPres:Lob_WinterSkate_SchoPres), mean, na.rm = TRUE)
...
pres_ovr_plots <- ggplot() +
  ...
  facet_wrap(~Metric, scales = "free_y") +
  theme_bw()
ggsave(pres_ovr_plots, file = paste(out_dir_use, "PresenceOverlapSummaries", ".jpg", sep = ""))
```
...
species_keep<- unique(tidy_mod_data$COMMON_NAME)
jsdm_dat<- tidy_mod_data %>%
  filter(., COMMON_NAME %in% species_keep) %>%
  mutate(., COMMON_NAME_Fac = factor(COMMON_NAME, levels = species_keep))

# Setting up VAST model -- some of this is redundant if the single species model has been run already
# Some helpful spatial files
menh_shp<- st_read(paste0(shapefile_root, "Shapefiles/MaineDMR_Inshore_Trawl_Survey_Grid/MaineDMR_Inshore_Trawl_Survey_Grid.shp"))
menh_chull<- menh_shp %>%
  st_union() %>%
...
vast_extrap_grid<- vast_make_extrap_grid(region_shapefile = menh_chull, index_shapes = menh_regions_out, cell_size = 5000) %>%
  drop_na(., STRATA)
...
```{r}
# Read in land shapefile, MA trawl, ME-NH trawl and NOAA NEFSC trawl footprints
land <- st_read(paste0(shapefile_root, "Shapefiles/ne_50m_admin_1_states_provinces/ne_50m_admin_1_states_provinces.shp")) %>%
  st_simplify(., dTolerance = 0.005)
noaa_nefsc <- st_read(paste0(shapefile_root, "Shapefiles/BottomTrawlStrata/BTS_Strata.shp")) %>%
  st_transform(., crs = st_crs(land))
me_nh <- st_read(paste0(shapefile_root, "Shapefiles/MaineDMR_Inshore_Trawl_Survey_Grid/MaineDMR_Inshore_Trawl_Survey_Grid.shp")) %>%
  st_transform(., crs = st_crs(land))
ma_trawl<- st_read(paste0(shapefile_root, "Shapefiles/MA_Trawl_Strata/ra_strata_poly.shp")) %>%
  st_transform(., crs = st_crs(land))

xlim <- c(-78.5, -62.5)
ylim <- c(32, 47)
land_color <- "#d9d9d9"
survey_domain_plot <- ggplot() +
  geom_sf(data = land, fill = land_color, lwd = 0.2, na.rm = TRUE) +
  geom_sf(data = noaa_nefsc, fill = NA, color = "#7570b3", lwd = 0.5) +
  geom_sf(data = me_nh, fill = "#1b9e77", color = "#1b9e77", alpha = 0.2, lwd = 0.1) +
  geom_sf(data = ma_trawl, fill = "#d95f02", color = "#d95f02", alpha = 0.2, lwd = 0.1) +
